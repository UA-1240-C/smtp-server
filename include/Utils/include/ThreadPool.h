...
private:
    /**
     * @brief Initialize the threads in the pool.
     ...
     */
    void ThreadLoop(std::size_t id, std::function<void(std::size_t)> init, const std::stop_token &stop_tok)
    {
        try
        {
            std::invoke(init, id);
    ...
    {
        return [func = std::move(f), ... largs = std::move(args), promise]()
        {
            try
            {
