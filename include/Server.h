...
class SmtpServer {
public:
    SmtpServer(boost::asio::io_context& io_context, unsigned short port, ThreadPool<>& pool);

private:
    void start_accept();
    void handle_session(std::shared_ptr<boost::asio::ssl::stream<boost::asio::ip::tcp::socket>> socket);
    void handle_request(std::shared_ptr<boost::asio::ssl::stream<boost::asio::ip::tcp::socket>> socket, bool is_tls);
    void save_mail(const MailData& mail_data);
