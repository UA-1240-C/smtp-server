...
void SmtpServer::handleStartTLS(SocketWrapper socket_wrapper) {
    ...
    auto tcp_socket = socket_wrapper.get<TcpSocket>();
    auto ssl_socket = std::make_shared<SslSocket>(std::move(*tcp_socket), ssl_context_);
    ...
    ssl_socket->async_handshake(
        ...
        [this, &socket_wrapper](const boost::system::error_code& error) {
            if (!error) {
                ...
                sendResponse(socket_wrapper, "454 TLS negotiation failed\r\n");
            }
        });*/
}

void SmtpServer::handleAuth(SocketWrapper socket_wrapper, const std::string& line) {
    /*std::string encoded_credentials = line.substr(11);
    std::string decoded_credentials = ISXBase64::Base64Decode(encoded_credentials);

    std::cout << "Decoded credentials: " << decoded_credentials << std::endl;

    auto delimiter_pos = decoded_credentials.find(':');
    if (delimiter_pos == std::string::npos) {
        sendResponse(socket_wrapper, "501 Syntax error in parameters or arguments\r\n");
        return;
    }
...
void SmtpServer::handleRegister(SocketWrapper socket_wrapper, const std::string& line) {
